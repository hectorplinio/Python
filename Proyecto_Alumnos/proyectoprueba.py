...
class Alumno(Persona):
    ...
    def inicializar_alumno(self,nombre,curso):
        self.nombre=nombre
        self.curso=curso
...
class Asignatura(Profesor):
    ...
    def inicializar_asignatura(self,idasignatura):
        self.idasignatura=idasignatura
    ...
    def alta_asignatura(self):
        ...
        self.curso.append(input("Dime el curso al que pertenece: "))
        self.horas_semanales.append(int(input("Dime el numero de horas semanales: ")))
        asignatura.horas_impartidas()
        self.horas_semanales[-1]=str(self.horas_semanales[-1])
        frase=self.idasignatura[-1]+";"+self.nombre[-1]+";"+self.identificador[-1]+";"+self.curso[-1]+self.horas_semanales[-1]
        archivo.write(frase+"\n")
        print("Ha añadido usted la asignatura "+self.nombre[-1]+" con un código "+self.idasignatura[-1]+" que la imparte el profesor con el codigo  "+self.identificador[-1]+" , su curso es  "+self.curso[-1]+" y tiene "+self.horas_semanales[-1]+" horas semanales.")
    ...
    def horas_impartidas(self):
        sumas_horas=0
        x=0
        contador=0
        for i in self.identificador:
            if i == self.identificador[-1]:
                x=self.horas_semanales[contador]
                x=int(x)
                sumas_horas=sumas_horas+x
                contador+=1
            else:
                contador+=1
        if sumas_horas >20:
            print("Este profesor no puede tener mas de 20 horas a la semana.")
            print("El profesor tiene un numero de horas de ",sumas_horas," corrija el numero de horas")
            hora=int(input("Dime el numero de horas semanales: "))
            if hora < self.horas_semanales[-1]:
                self.horas_semanales[-1]=(hora)
            else:
                hora=int(input("ERROR, Dime el numero de horas semanales: "))




class Notas(Asignatura, Alumno):
    def __init__(self,idasignatura, nombre, curso, nota):
        Alumno.inicializar_alumno(self, nombre, curso)
        Asignatura.inicializar_asignatura(self,idasignatura)
        self.nota=nota
    
    def alta_nota(self):
        for i in lista_curso:
            print(i)
        curso=int(input("Debe elegir primero el curso al que quiere añadir las notas:"))
        semaforo_curso=True
        while semaforo_curso == True:
            if curso == 1:
                self.curso.append(curso)
                semaforo_curso=False
            elif curso == 2:
                self.curso.append(curso)
                semaforo_curso=False
            elif curso == 3:
                ...
                curso = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
    
        for i in lista_asignaturas:
            print(i)
        asig=int(input("Dime a que asignatura quiere ponerle las notas: "))
        semaforo_nota=True       
        archivo = open("Notas.txt", "a+")
        while semaforo_nota == True:
            if asig == 1 :
                print("Vas a poner las notas Matematicas. ")
                for i in alumno.nombre:
                    calificacion = input(i+":")
                    self.nota.append(calificacion)
                    self.nombre.append(alumno.nombre)
                    self.idasignatura.append(asig)
                    self.curso.append(curso)
                    frase="1"+";"+i+";"+calificacion+";"+str(curso)+";Matematicas"
                    archivo.write(frase+"\n")
                archivo.close()
                semaforo_nota=False
            elif asig == 2 :
                print("Vas a poner las notas Ingles.")
                for i in alumno.nombre:
                    calificacion = input(i+":")
                    self.nota.append(calificacion)
                    self.nombre.append(alumno.nombre)
                    self.idasignatura.append(asig)
                    self.curso.append(curso)
                    frase="1"+";"+i+";"+calificacion+";"+str(curso)+";Ingles"
                    archivo.write(frase+"\n")
                archivo.close()
                semaforo_nota=False
            elif asig == 3 :
                print("Vas a poner las notas Informatica.")
                for i in alumno.nombre:
                    calificacion = input(i+":")
                    self.nota.append(calificacion)
                    self.nombre.append(alumno.nombre)
                    self.idasignatura.append(asig)
                    self.curso.append(curso)
                    frase="1"+";"+i+";"+calificacion+";"+str(curso)+";Informatica"
                    archivo.write(frase+"\n")
                archivo.close()
                semaforo_nota=False
            elif asig == 4 :
                print("Vas a poner las notas Historia.")
                for i in alumno.nombre:
                    calificacion = input(i+":")
                    self.nota.append(calificacion)
                    self.nombre.append(alumno.nombre)
                    self.idasignatura.append(asig)
                    self.curso.append(curso)
                    frase="1"+";"+i+";"+calificacion+";"+str(curso)+";Historia"
                    archivo.write(frase+"\n")
                archivo.close()
                semaforo_nota=False
            else:
    ...
    def ver_notas(self):
        for i in alumno.nombre:
            print(i)
        nom=input("Dime el nombre del alumno que quieres ver las notas: ")
        nom=nom.capitalize()
        semaforo_ver=True
        archivo=open("Boletin.txt", "w")
        while semaforo_ver == True:
            for i in alumno.nombre:
                if nom == i:
                    archivo2 = open("Notas.txt", "r")
                    print("Estas son las notas de "+nom+" :")
                    archivo.write("Estas son las notas de "+ nom+" :"+"\n")
                    for lineas in archivo2:
                        lineas=lineas.strip("\n")
                        lineas=lineas.split(";")
                        if lineas[1] == nom:
                            print(lineas[4]+": "+lineas[2])
                            archivo.write(lineas[4]+": "+lineas[2]+"\n")
                        else:
                            pass
                    archivo.close()
                    archivo2.close()
        ...
        if opcion == 1:
            ...
            while semaforo_alta == True:
                if opcion_alta == 1:
                    ...
                    opcion_alta = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
        elif opcion == 2:
            for i in lista_baja:
                print(i)
            semaforo_baja=True
            ...
            while semaforo_baja == True:
                if opcion_baja == 1:
                    ...
                    opcion_baja = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
        elif opcion == 3:
            ...
            while semaforo_modificar == True:
                if opcion_modificar == 1:
                    ...
                    opcion_modificar = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
        elif opcion == 4:
            ...
            while semaforo_listado == True:
                if opcion_listado == 1:
                    ...
                    opcion_listado = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
        elif opcion == 5:
            for i in lista_listado:
                print (i)
            semaforo_buscar=True
            ...
            while semaforo_buscar == True:
                if opcion_buscar == 1:
                    ...
                    opcion_buscar = int(input("Esa opcion no es valida, ponga una opcion correcta :"))
        elif opcion == 6:
            for i in lista_notas:
                print(i)
            semaforo_notas=True
            opcion_notas=int(input("Dime que opcion eliges: "))
            while semaforo_notas == True:
                if opcion_notas == 1:
                    notas.alta_nota()
                    semaforo_notas=False
                elif opcion_notas == 2:
                    notas.ver_notas()
                    semaforo_notas=False
                elif opcion_notas == 3:
                    ...
                    opcion_notas=int(input("Esa opcion no es valida, ponga una opcion correcta: "))
        elif opcion == 7:
            semaforo = False
        else:
...
lista_menu=["1- Dar de alta", "2- Dar de baja", "3- Modificar parametro" , "4- Listado ", "5- Buscar", "6- Notas", "7- Salir"]
...
lista_asignaturas=["1- Matematicas", "2- Ingles", "3- Informatica","4- Historia", "5- Salir"]
lista_notas=["1- Poner notas", "2- Ver notas alumno", "3- Salir"]
lista_curso=["1- Primero", "2- Segundo", "3- Tercero", "4- Salir"]
